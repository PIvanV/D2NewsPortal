...
def notify_subscribers(sender, instance, created, **kwargs):
    ...
    if instance.postCategory.first():
        ...
        for subscriber in subscribers:
            ...
            if created:
                subject = f'{subscriber.username}, новая публикация - {instance.title}, в разделе {instance.postCategory.first()} ... {instance.dateCreation.strftime("%d %m %Y")}'
            else:
                subject = f'{subscriber.username}, новая публикация - {instance.title}, в разделе {instance.postCategory.first()} ... {instance.dateCreation.strftime("%d %m %Y")}'

            msg = EmailMultiAlternatives(
                subject=subject,
                body=f'Привет {subscriber.username}, новая публикация - {instance.title}, в разделе {instance.postCategory.first()}',  # это то же, что и message
                from_email='NewsPaperD6@yandex.ru',
                to=[f'{subscriber.email}'],  # это то же, что и recipients_list
            )

            # получаем наш html
            html_content = render_to_string(
                'sample_app/post_create.html',
                {
                    'post': instance,
